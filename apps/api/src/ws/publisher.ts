// apps/api/src/ws/publisher.ts
import { WebSocket } from "ws";
import { getSignalingInfo } from "../utils/awsSigner";

export default async function setupPublisherWs(wsClient: WebSocket, req: any) {
  const url      = new URL(req.url!, `http://${req.headers.host}`);
  const clientId = url.searchParams.get("clientId");
  if (!clientId) {
    console.error("[WS publisher] Missing clientId");
    return wsClient.close();
  }

  // MASTER role + clientId
  let wssSignedUrl: string, iceServers: any[];
  try {
    ({ wssSignedUrl, iceServers } = await getSignalingInfo(clientId, "MASTER", clientId));
  } catch (err) {
    console.error("[WS publisher] signer failed:", err);
    return wsClient.close();
  }
  ...
  // 3) Buffer client→AWS hasta que abra
  const pending: any[] = [];
  wsAws.on("open", () => {
  ...
  // 4) Proxy AWS→browser inmediato
  wsAws.on("message", msg => wsClient.send(msg));

  // 5) Cleanup en close/error
